...
echo "Notary RequestID = "$REQUEST_ID
...
printf "\nNotary Request id = $REQUEST_ID   CPI hash = $CPI_HASH   Status = $STATUS\n\n"
sleep 1
...
sleep 1
read REQUEST_ID <<< $(curl --insecure -u admin:admin  -s -F upload=@$CPI https://localhost:8888/api/v1/cpi/ | jq -r '.id')
echo "VNode 1 RequestID = "$REQUEST_ID
...
echo "VNode 1 Holding ID = "$HOLDING_ID
...
printf "\nVNode 2 Request id = $REQUEST_ID   CPI hash = $CPI_HASH   Status = $STATUS\n\n"
...
echo "VNode 2 Holding ID = "$HOLDING_ID_2
