...
import org.abi.TypeReference;
import org.abi.Utils;
import org.abi.datatypes.*;
import org.abi.datatypes.generated.Uint160;
import org.abi.datatypes.primitive.Double;
import org.abi.datatypes.primitive.Float;
...
import static org.abi.DefaultFunctionReturnDecoder.getDataOffset;
import static org.abi.TypeReference.makeTypeReference;
import static org.abi.Utils.*;
...
public class TypeDecoder {
    ...
    public static Type instantiateType(org.abi.TypeReference ref, Object value)
    ...
    public static <T extends Array> T decode(
            String input, int offset, org.abi.TypeReference<T> typeReference) {
    ...
    static Type instantiateArrayType(org.abi.TypeReference ref, Object value)
            throws NoSuchMethodException, IllegalAccessException, InvocationTargetException,
                    InstantiationException, ClassNotFoundException {
        List values;
        ...
        int arraySize =
                ref instanceof org.abi.TypeReference.StaticArrayTypeReference
                        ? ((org.abi.TypeReference.StaticArrayTypeReference) ref).getSize()
                        : -1;
        if (arraySize <= 0) {
            listcons = DynamicArray.class.getConstructor(Class.class, List.class);
        } else {
            Class<?> arrayClass =
                    Class.forName("org.abi.datatypes.generated.StaticArray" + arraySize);
        ...
        org.abi.TypeReference subTypeReference = ref.getSubTypeReference();
        for (Object o : values) {
    ...
    public static <T extends Type> T decodeStaticArray(
            String input, int offset, org.abi.TypeReference<T> typeReference, int length) {
    ...
    public static <T extends Type> T decodeStaticStruct(
            final String input, final int offset, final org.abi.TypeReference<T> typeReference) {
        BiFunction<List<T>, String, T> function =
                (elements, typeName) -> {
                    if (elements.isEmpty()) {
                        throw new UnsupportedOperationException(
    ...
    private static <T extends Type> T decodeStaticStructElement(
            final String input,
            final int offset,
            final org.abi.TypeReference<T> typeReference,
        ...
        try {
                ...
                if (StaticStruct.class.isAssignableFrom(declaredField)) {
                    ...
                    value =
                            decodeStaticStruct(
                                    ...
                                    0,
                                    org.abi.TypeReference.create(declaredField));
            ...
            throw new UnsupportedOperationException(
                    "Unable to access parameterized type "
                            + org.abi.Utils.getTypeName(typeReference.getType()),
                    e);
        }
    }
    ...
    private static <T extends Type> T instantiateStruct(
            final org.abi.TypeReference<T> typeReference, final List<T> parameters) {
        try {
            Class<T> classType = typeReference.getClassType();
            ...
            for (int i = 0; i < length; ++i) {
                ...
                if (isDynamic(declaredField)) {
                    ...
                    final Class<T> parameterFromAnnotation =
                            org.abi.Utils.extractParameterFromAnnotation(
    ...
    private static <T extends Type> T decodeDynamicParameterFromStruct(
        ...
        if (DynamicStruct.class.isAssignableFrom(declaredField)) {
            value = decodeDynamicStruct(dynamicElementData, 0, org.abi.TypeReference.create(declaredField));
            ...
            value =
                    (T)
                            decodeDynamicArray(
                                    dynamicElementData,
                                    0,
                                    org.abi.Utils.getDynamicArrayTypeReference(parameter));
        } else {
        ...
        try {
            Class<? extends StaticArray> arrayClass =
                    (Class<? extends StaticArray>)
                            Class.forName("org.abi.datatypes.generated.StaticArray" + length);
    ...
    private static <T extends Type> T decodeArrayElements(
            String input,
            int offset,
            org.abi.TypeReference<T> typeReference,
            int length,
            BiFunction<List<T>, String, T> consumer) {

        try {
            Class<T> cls = org.abi.Utils.getParameterizedTypeFromArray(typeReference);
            List<T> elements = new ArrayList<>(length);
            if (StructType.class.isAssignableFrom(cls)) {
                    ...
                    if (DynamicStruct.class.isAssignableFrom(cls)) {
                        value =
                                TypeDecoder.decodeDynamicStruct(
                                        ...
                                        org.abi.TypeReference.create(cls));
                    } else {
                        value =
                                TypeDecoder.decodeStaticStruct(
                                        input, currOffset, org.abi.TypeReference.create(cls));
                    }
                    elements.add(value);
                }
                ...
                for (int i = 0, currOffset = offset; i < length; i++) {
                    ...
                    if (DynamicArray.class.isAssignableFrom(cls)) {
                        value =
                                (T)
                                        TypeDecoder.decodeDynamicArray(
                                                ...
                                                org.abi.Utils.getDynamicArrayTypeReference(
                                                        org.abi.Utils.getFullParameterizedTypeFromArray(
                                                                typeReference)));
                        currOffset +=
                                getSingleElementLength(input, currOffset, cls)
                                        * MAX_BYTE_LENGTH_FOR_HEX_STRING;
                        ...
                        org.abi.TypeReference innerType =
                                org.abi.TypeReference.create(
                                        org.abi.Utils.getFullParameterizedTypeFromArray(typeReference));

                        org.abi.TypeReference.StaticArrayTypeReference staticReference =
                                new org.abi.TypeReference.StaticArrayTypeReference<StaticArray>(
                                        staticLength) {
