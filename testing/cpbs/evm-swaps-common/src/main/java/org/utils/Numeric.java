...
public final class Numeric {
    ...
    public static String encodeQuantity(BigInteger value) {
        if (value.signum() != -1) {
            ...
            throw new CordaRuntimeException("Negative values are not supported");
        }
    }
    ...
    public static BigInteger decodeQuantity(String value) {
        ...
        if (!isValidHexQuantity(value)) {
            throw new CordaRuntimeException("Value must be in format 0x[0-9a-fA-F]+");
        }

        try {
            return parsePaddedNumberHex(value);
        } catch (NumberFormatException e) {
            throw new CordaRuntimeException("Negative ", e);
        }
    }
