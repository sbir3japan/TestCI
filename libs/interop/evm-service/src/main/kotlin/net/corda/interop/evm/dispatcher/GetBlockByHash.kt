...
import net.corda.interop.evm.constants.ETH_GET_BLOCK_BY_HASH
import net.corda.data.interop.evm.response.Block


/**
 * Dispatcher used to Get a Block By Hash
 *
 * @param evmConnector The evmConnector class used to make rpc calls to the node
 */
class GetBlockByHash(private val evmConnector: EthereumConnector) : EvmDispatcher {
    override fun dispatch(evmRequest: EvmRequest): EvmResponse {
        val request = evmRequest.payload as GetBlockByHash
        // EVM Expects both data & input
        val resp = evmConnector.send<EthereumBlock>(
            evmRequest.rpcUrl,
            ETH_GET_BLOCK_BY_HASH,
            listOf(request.blockHash, request.fullTransactionObjects)
        )
